...
def recondestination(dbclient,main_staging_collection,main_table_collection,recon_threshold_count,logger):
 ...
 def delete_old_records(main_table_collection, logger):
    """Deletes records from main_table_collection where timestamp is older than 90 days"""
    ninety_days_ago = datetime.now() - timedelta(days=RETENTION_DAYS)

    result = main_table_collection.delete_many({
        "timestamp": {"$lt": ninety_days_ago}
    })

    logger.info(f"Deleted {result.deleted_count} records older than 90 days from main table.")